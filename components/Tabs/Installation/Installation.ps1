...
function Invoke-installRequirements {
  $RandomCode = New-Guid
  invoke-CreateLogs $installationLogs
  installRequirements
...
function installRequirements {
  ...
  foreach ($name in $sortedRequirements) {
    # Fix temporaneo per mantenere un ordinamento  basato sulle dipendenze anche durante la fase di installazione
    ...
    switch ($name) {
      ...
      "Npm Login" {
        $requirement["InstallCommand"] | Invoke-Expression
        break
      }
      "Install CAEP" {
        $requirement["InstallCommand"] | Invoke-Expression
  ...
  Write-Host "`r`nEnv var Path reloaded correctly"
  Invoke-CreateRow $gridInstallation $name $green
}
  
function invoke-wsl($name, $requirement) {

  if ($requirement["Result"].Contains("VERSION")) {
    Invoke-Expression wsl --set-default-version 2
  }

  if ($requirement["Result"].Contains("UBUNTU")) {
    wsl --install -d Ubuntu-Requirement['MaxVersion']
  }

  if ($requirement["Result"].Contains("MAIN")) {
   
    if (-not $requirement["Result"].Contains("VER AVAILABLE")) {
      wsl --install -d Ubuntu-Requirement['MaxVersion']
    }
    ...
    foreach ($version in (wsl -l -v).split('')) {
...
function invoke-visualStudioCode($name, $requirement) {
  $isKo = $checkLogs[$Name]["Result"].Contains("KO")
  $isKoOrVer = $isKo -or $checkLogs[$Name]["Result"].Contains("VER")

  if ($isKo) {
    Invoke-DownloadInstall $name, $requirement
  }

  if ($isKoOrVer -or $checkLogs[$name]["Result"].Contains("SETTINGS")) {
    invoke-WriteLogs $installationLogs "Updating Visual Studio Code Settings:``r``n- Default Integrated Terminal: Command Prompt``r``n- Update Mode: Manual...`\"
    ...
    invoke-WriteLogs $installationLogs "return 'Update of Visual Studio Code Settings complete.'"
  }

  if ($isKoOrVer -or $checkLogs[$Name]["Result"].Contains("EXTENTIONS")) {
    invoke-WriteLogs $installationLogs "Installing Visual Studio Code Extentions..."

    foreach ($item in $requirement["Attributes"]) {

      invoke-WriteLogs $installationLogs "Installing $item..."
      ...
      if (-not $resultInstallation -or $contentErrLogfile -like "*Failed Installing Extensions*") {
        ...
        invoke-WriteLogs $installationLogs "Failed installing extension $item!"
      } 
      elseif ($contentErrLogfile -like "*was successfully installed.*") {
        $outputInstallationLabel.SelectionColor = "Green"
        invoke-WriteLogs $installationLogs "$item was successfully installed."
      }
    }  

    invoke-WriteLogs $installationLogs "Install of Visual Studio Code Extensions complete."
  }
}
...
function Invoke-DownloadInstall($name, $requirement) {
  ...
  if ($requirement['DownloadCommand']) {
    invoke-WriteLogs $installationLogs "Downloading $subMessage..."
    if ($name -eq "Setup CATools") {
      Invoke-RestMethod (((Invoke-RestMethod $requirement["DownloadLink"]).assets[0]).browser_download_url) -OutFile $requirement["DownloadOutfile"]
    }
    else {
      Invoke-RestMethod $requirement["DownloadLink"] -OutFile $requirement["DownloadOutfile"]
    }
    invoke-WriteLogs $installationLogs "Download of $subMessage complete."
  }

  if ($requirement['InstallCommand']) {
    invoke-WriteLogs $installationLogs "Installing $subMessage..."
    # $requirement["InstallCommand"] | Invoke-Expression
    invoke-WriteLogs $installationLogs "Install of $subMessage complete."
  }

  if ($requirement['DeleteCommand']) {
    invoke-WriteLogs $installationLogs "Deleting the file $($requirement["DownloadOutfile"])..."
    Remove-Item ($requirement["DownloadOutFile"].replace('"', ''))
    invoke-WriteLogs $installationLogs "Delete of the file $($requirement["DownloadOutfile"]) complete."
  }
}
...
function invoke-npm($name, $requirement) {
  ...
  if ($isKo -or $checkLogs[$Name]["Result"].Contains("PROXY")) {
