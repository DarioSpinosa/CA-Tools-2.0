function Invoke-Main {
  ...
  foreach ($Name in $Requirements.Keys) {
    ...
    $Version = If ($Requirements[$Name]["MaxVersion"]) { ", Version = $($Requirements[$Name]["MaxVersion"])" } else { "" }
    ...
    switch ($Requirement["Type"]) {
      "Software" {
        Invoke-DownloadInstallRequirementAction $Requirement
      }
      "Feature" {
        Invoke-EnableFeatureAction $Requirement
      }
      "Env Variable Path" {
      ...
      "Permission" {
        Invoke-PermissionAction $Requirement
      }
      "Connection" {
      ...
      "Activity" {
        Invoke-ActivityAction $Requirement
      }
      Default {
    ...
    if ($Requirement["PostAction"]) {
      Show-Buttons @("$($Requirement.PostAction)Button")
    }
  }
}
...
function Invoke-ActivityAction {
  ...
  if ($ResultActivity[0]) {
    switch ($ResultActivity[1]) {
      'OK' {
        if (!($Requirement.ActivityOKCommand)) { return }
      ...
      Default {
        invoke-writeOutputInstallations("There's an anomaly in Activity Invocation!")
      }
    }
  ...
  if ($ResultConnection[0]) {
    switch ($ResultConnection[1]) {
      "OK" {
        ...
        invoke-writeOutputInstallations($MessageConnection)
        invoke-writeOutputInstallations([Environment]::NewLine)
      }
    }
    Show-Buttons @('$NextButton', '$CancelButton')
  }
...
function Invoke-DownloadInstallRequirementAction {
  ...
  if ($Requirement.CheckRequirement) {
    invoke-writeOutputInstallations("$CheckingMessage")
  ...
  if (($IsInstalled[1] -ne 'OK') -or ($Requirement.PostAction -eq "Reinstall")) {
    ...
    if ($Requirement.DownloadCommand) {
      invoke-writeOutputInstallations("$DownloadingMessage")
      Invoke-Expression (New-CommandString $Requirement.DownloadCommand)
      invoke-writeOutputInstallations("$DownloadCompleteMessage")
    }
    if ($Requirement.InstallCommand) {
      invoke-writeOutputInstallations("$InstallingMessage")
      Invoke-Expression (New-CommandString $Requirement.InstallCommand)
      invoke-writeOutputInstallations("$InstallCompleteMessage")
    }
    if ($Requirement.DeleteCommand) {
      invoke-writeOutputInstallations("$DeletingMessage")
      Invoke-Expression (New-CommandString $Requirement.DeleteCommand)
      invoke-writeOutputInstallations("$DeleteCompleteMessage")
    }
  }
  else {
    invoke-writeOutputInstallations("$AlreadyInstalledMessage")
  }
  Show-Buttons @('$NextButton', '$CancelButton')
}
...
function Invoke-EnableFeatureAction {
  ...
  if ($Requirement.CheckRequirement) {
    ...
    if ($EnabledResult[1] -eq 'KO') {
      invoke-writeOutputInstallations("$EnablingMessage")
      Invoke-Expression (New-CommandString $Requirement.EnableCommand)
      invoke-writeOutputInstallations("$EnableCompleteMessage")
    }
    else {
      invoke-writeOutputInstallations("$AlreadyEnabledMessage")
    }
  }
  Show-Buttons @('$NextButton', '$CancelButton')
}
...
function Invoke-PermissionAction {
  ...
  if ($ResultPermission[0]) {
    if ($ResultPermission[1] -eq 'OK') {
      ...
      $outputInstallationLabel.SelectionLength = 0
      $outputInstallationLabel.SelectionColor = "Green"
      invoke-writeOutputInstallations($MessagePermission)
      invoke-writeOutputInstallations([Environment]::NewLine)
    }
    else {
      ...
      if ($Requirement.CheckRequirement) {
        ...
        if (-not $ResultCommand) {
          ...
          elseif ($ContentErrLogfile -like "*was successfully installed.*") {
            ...
            $outputInstallationLabel.SelectionLength = 0
            $outputInstallationLabel.SelectionColor = "Green"
            invoke-writeOutputInstallations("$item was already installed.")
            invoke-writeOutputInstallations([Environment]::NewLine)
          }
        }
      }
    ...
    if ($Requirement.CheckRequirement) {
      ...
      if ($PostInstallResultCheck[0]) {
        if ($PostInstallResultCheck[1] -eq 'OK') {
          ...
          $outputInstallationLabel.SelectionLength = 0
          $outputInstallationLabel.SelectionColor = "Green"
          invoke-writeOutputInstallations($MessagePostInstall)
          invoke-writeOutputInstallations([Environment]::NewLine)
...
function Invoke-PreInstallAction {
  ...
  invoke-writeOutputInstallations($MessagePreInstall)
  invoke-writeOutputInstallations([Environment]::NewLine)
}
