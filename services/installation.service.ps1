#--------------------------------------------------------[FUNCTIONS]--------------------------------------------------------
function installRequirement($name) {
  <#
    .SYNOPSIS
  ...
  if ($requirement["PostAction"]) {
    ...
    $postActionButton.Text = $requirement["PostAction"]
  }
  else {
...
function Invoke-DownloadInstall($name, $requirement) {
  ...
  if ($requirement['DownloadCommand']) {
    invoke-writeOutputInstallations("Downloading $subMessage...")
    # $requirement["DownloadCommand"] | Invoke-Expression
    invoke-writeOutputInstallations("Download of $subMessage complete.")
  }

  if ($requirement['InstallCommand']) {
    invoke-writeOutputInstallations("Installing $subMessage...")
    # $requirement["InstallCommand"] | Invoke-Expression
    invoke-writeOutputInstallations("Install of $subMessage complete.")
  }

  if ($requirement['DeleteCommand']) {
    invoke-writeOutputInstallations("Deleting the file $($requirement["DownloadOutfile"])...")
    # $requirement["DeleteCommand"] | Invoke-Expression
    invoke-writeOutputInstallations("Delete of the file $($requirement["DownloadOutfile"]) complete.")
  }
}
...
function Invoke-PostInstallAction($requirement) {
  if ($requirement["Result"] -eq 'KO') {
    ...
    $requirement['PostInstallCommand'] | Invoke-Expression
    invoke-writeOutputInstallations(($requirement['PostInstallCompleteMessage'] | Invoke-Expression))
  }
  else {
    #CASE "TCP" returned only proxy-docker and proxy-npm (two script differs only for 1 line)
    $outputInstallationLabel.SelectionColor = "Red"
...
function Invoke-ActivityAction($name, $requirement) {
  $requirement["ActivityKOCommand"] | Invoke-Expression
}
...
function Invoke-LoginNpm {
  ...
  if (!($UsernameTextBox.Text -ne "") -or !($TokenTextBox.Text -ne "")) {
    $outputInstallationLabel.SelectionColor = "Red"
    invoke-writeOutputInstallations("Username and Token can't be NULL! Please enter the Username and Password.")
    return
  }
  ...
  if ($doubleCheck) {
    $outputInstallationLabel.SelectionColor = "Red"
    invoke-writeOutputInstallations("Npm Login Error!`r`nThe Token or the Username are wrong. Check again your Username and Token.`r`nPS: Be sure that the token is setted as 'All Accessible Organization'.")
    return
  }
...
function Show-NpmLoginError($Msg) {
  ...
  $outputInstallationLabel.SelectionStart = $outputInstallationLabel.TextLength
  $outputInstallationLabel.SelectionLength = 0
  $outputInstallationLabel.SelectionColor = "Red"
  invoke-writeOutputInstallations($Msg)
}
...
function Close-Installer {
  ...
  try {
    ...
    invoke-writeOutputInstallations("No process running on port 4200")
  }
  Remove-StartupCmd
